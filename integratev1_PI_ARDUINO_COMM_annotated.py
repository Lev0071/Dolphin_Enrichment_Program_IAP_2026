...
### PI_ARDUINO_COMM - OPEN_I2C_BUS - Open Raspberry Pi I²C bus #1 (all Arduino I²C traffic uses this handle)
...
Solenoid2Pin = 12
Solenoid1Pin = 16
### PI_ARDUINO_COMM - PROTOCOL - Handshake/presence-check byte sent to each Arduino during scan
HELLO_SIGNAL = 0xFF
Solenoid4Line = 14
Solenoid3Line = 15
Solenoid2Line = 18
...
### PI_ARDUINO_COMM - SCAN_I2C_ADDRESSES - Candidate Arduino I²C addresses to probe (0x08..0x18)
...
class Button:
    ...
    def checkbuttonstatus ( self ):
        ...
        if not self.override: # don't check if override is set        
            try:
                ### PI_ARDUINO_COMM - RX - BUTTON_STATE - Read 1 byte: expected 0=unpressed, 1=pressed
    ...
    def updateLED ( self ):
        try:
            ### PI_ARDUINO_COMM - TX - LED_SOUND_COMMAND - Write 1 control byte to set LED + sound on Arduino
            ### PI_ARDUINO_COMM - ENCODING - LED_SOUND_BYTE - Byte = (LEDstatus*LEDtype) + 4*(soundstatus*soundfreq)
...
def is_arduino_on(address, bus):
    try:
        ### PI_ARDUINO_COMM - TX - HANDSHAKE_HELLO - Send HELLO (0xFF) to probe Arduino presence
        bus.write_byte(address, HELLO_SIGNAL)
        time.sleep(0.05)
        ### PI_ARDUINO_COMM - RX - HANDSHAKE_REPLY - Read 1 byte back (any successful read implies device responded)
...
def reset_arduino():
    ...
    sys.stdout.flush()
#    GPIO.output(Solenoid2Pin, GPIO.HIGH)
    ### PI_ARDUINO_COMM - RESET_POWER - POWER_OFF - Cut power to Arduino boards (used to recover when comms get stuck)
...
#    GPIO.output(Solenoid2Pin, GPIO.LOW)
    ...
    ### PI_ARDUINO_COMM - RESET_POWER - POWER_ON - Restore power, then rescan I²C for boards
    ### PI_ARDUINO_COMM - RESET_POWER - STARTUP_POWER_ON - Power Arduino boards on before initial scan
...
def perform_scan():
    ...
    index=0
    ### PI_ARDUINO_COMM - SCAN - Loop through candidate addresses and run handshake (TX+RX)
    for address in I2C_addresses:
        print(address)
        if is_arduino_on(address, I2Cbus):
            ### PI_ARDUINO_COMM - BIND - Wrap detected Arduino address into a Button object (stores address + bus)
...
def main(args):
    ...
    ### PI_ARDUINO_COMM - RESET_POWER - SHUTDOWN_POWER_OFF - Power Arduino boards off on program exit
    powerLine.set_value ( Solenoid2Line, gpiod.line.Value.ACTIVE )
